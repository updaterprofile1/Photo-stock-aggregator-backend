...
const { recordSubmissionHistory } = require('./thumbnailPersistence');
...
async function submitAssets({ assetIds, siteSlug, userId }) {
  ...
  const invalidSubmitTransitions = assets.filter((asset) => !canTransition(asset.status, 'submitted'));
  if (invalidSubmitTransitions.length) {
    const err = new Error(
      `Assets not ready for submission: ${invalidSubmitTransitions.map((asset) => asset.id).join(', ')}`
    );
  ...
  // ── 4. Persist job record (queued) before calling provider ─────────────
  ...
  // ── 5. Call submission provider (mock or n8n) ────────────────────────────
  const provider = chooseSubmissionProvider();
  let result;
  try {
    result = await provider({ assets, siteSlug: normalizedSiteSlug, userId });
  } catch (providerErr) {
    // Mark affected assets and job as rejected
    const canMarkRejected = assets.every((asset) => canTransition(asset.status, 'rejected'));
    const rejectedState = canMarkRejected ? 'rejected' : assets[0].status;
    await prisma.asset.updateMany({
      where: { id: { in: assetIds } },
      data: { status: rejectedState },
    });
    ...
    await Promise.all(
      assets.map((asset) =>
        recordSubmissionHistory(asset.id, normalizedSiteSlug, { status: 'rejected' }, { userId })
      ),
    );
  ...
  await prisma.submissionJob.update({
    where: { id: jobId },
    data: { status: 'submitted', provider: result.provider },
  });

  await Promise.all(
    assets.map((asset) =>
      recordSubmissionHistory(asset.id, normalizedSiteSlug, { status: 'submitted' }, { userId })
    ),
  );
